...
void controlMotorSpeed(void *pvParameters) {
  for (;;) {
    if (xSemaphoreTake(encoderSemaphore, portMAX_DELAY) == pdTRUE) {
      readEncoder();
      motorSpeed = encoderCount * (1000.0 / 500.0); // Calculate motor speed based on encoder counts
      xSemaphoreGive(encoderSemaphore);
    }
...
void setup() {
  ...
  // Create tasks
  xTaskCreate(readPotentiometer, "PotentiometerTask", 1000, NULL, 1, NULL);
  xTaskCreate(readSerialParams, "SerialParamsTask", 1000, NULL, 1, NULL);
  xTaskCreate(controlMotorSpeed, "MotorControlTask", 1000, NULL, 1, NULL);
